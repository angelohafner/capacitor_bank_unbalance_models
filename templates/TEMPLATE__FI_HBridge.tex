...
\begin{figure}[htbp]
	...
	\label{fig:Figure36-h_bridge_internal_fuses}
\end{figure}

\noindent onde:

\begin{itemize}
	\item \textbf{S} é o número \textbf{total} de \textbf{grupos em série}.
	\item \textbf{St} é o número de \textbf{grupos em série} da perna ``H'' (High) até o neutro (ponto de referência).
	\item \textbf{Pt} = é o número \textbf{total} de \textbf{unidades em paralelo por fase}.
	\item \textbf{Pa} é o número de \textbf{unidades em paralelo} no lado ``esquerdo'' da perna ``H''.
	\item \textbf{P} é o número de \textbf{unidades em paralelo} na \textbf{string afetada} (provavelmente devido a uma falha ou análise).
	\item \textbf{G} é o estado de aterramento/ligação à terra ($\mathbf{0}$ = \textbf{aterrado}, $\mathbf{1}$ = \textbf{não aterrado/desligado da terra}).
	\item \textbf{N} é o número de \textbf{elementos em paralelo em um grupo} (série).
	\item \textbf{Su} é o número de \textbf{grupos em série} em uma \textbf{unidade capacitiva}.
\end{itemize}
...
\begin{table}[htbp]
	...
	\input{./tables/tabela_dados_nominais.tex}
	\label{tab:tabela8_real}
\end{table}
