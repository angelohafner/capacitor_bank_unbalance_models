...
def get_dados_nominais_banco(topologia: str):
    ...
    st.subheader("Dados Nominais do Banco de Capacitores")
    ...
    with colA:
        tensao_trifasica_banco_V = 1e3 * st.number_input(
            label="Tensão trifásica nominal (kV):",
            min_value=1.0, max_value=1000.0, value=69.0
        )
    with colB:
        potencia_trifasica_banco_VAr = 1e6 * st.number_input(
            label="Potência trifásica nominal (MVAr):",
    ...
    if topologia == "yy_internal_fuses":
        ...
        with c3:
            N  = st.number_input("Elementos em série por célula (N):", 1, 50, 14)
            Su = st.number_input("Células em série por unidade (Su):", 1, 10, 3)
